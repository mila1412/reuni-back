...
export default async (req, res, next) => {
  upload.fields([{ name: 'image1', maxCount: 1 }, { name: 'image2', maxCount: 1 }, { name: 'image3', maxCount: 1 }, { name: 'image4', maxCount: 1 }])(req, res, async error => {
    if (error instanceof multer.MulterError) {
      ...
      // req.file 是傳入的檔案資訊
      if (req.files.image1) {
        req.filepath1 = process.env.FTP ? path.basename(req.files.image1[0].path) : req.files.image1[0].filename
      }
      if (req.files.image2) {
  ...
  //         file.filepath = process.env.FTP ? path.basename(file.path) : file.filename
  //         return file
  //       })
  //       for (let i = 0; i < req.files.length; i++) {
  //         req.filepath = req.files[i].filepath
  //       }
  //       console.log(req.files[0].filepath)
